...
it('triggers spectrogramCache.analyse when audio buffer exists and not cached', async () => {
  ...
  await waitFor(() => {
    expect(mockAnalyse).toHaveBeenCalledWith(
      ...
      // The active effects in chain order (none here), the four macro
      // amounts, then the echo's resolved delay time — unsynced here, so
      // the fixed default (spec 007 M4, #560).
      '[]:0:0:0:0:0.250',
      expect.any(Function),
    );
  });
