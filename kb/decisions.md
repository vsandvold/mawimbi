...
## 2026-07-22 — Spectrogram stats bridge is a module-level singleton, not an AudioService-injected instance; `SpectrogramCache.setEntry` is its single write chokepoint for per-track data

**Decision:** `SpectrogramStats` (spec 006 milestone 1, #538) is instantiated once as a module-level `export const spectrogramStats = new SpectrogramStats()` in its own file, imported directly by `SpectrogramCache.ts`, `Spectrogram.tsx`, and `FrequencyVisualizer.ts` — not constructed by `AudioService` and threaded through constructors/props to those call sites. Per-track tile/frame byte counts and analysis timing are recorded from one place, `SpectrogramCache.setEntry` (every path that lands tiles — fresh analysis, IndexedDB restore, effects-refresh commit — converges there). `SpectrogramCache.analyse()` calls a paired `recordAnalysisStart(trackId)` that returns a token, threaded through to `setEntry`/`recordEntry`, so a genuinely concurrent second `analyse()` for the same track (an effects commit landing mid-analysis, `useSpectrogramCache.ts`'s documented re-entry case) can't consume or clear the other call's pending start — a mismatched or absent token reports `analysisMs: 0` rather than corrupting the wrong call's timing. `restore()` and effects-refresh's direct `setEntry` calls pass no token at all, so their own `analysisMs` reads 0 — but their `firstTileMs` is *not* 0 when a prior entry exists for that track: it's preserved from the original analysis (`this.tracks.get(trackId)?.firstTileMs ?? elapsedMs`), which is exercised today by the effects-refresh flow (upload → dry analysis → later effects commit re-enters `setEntry`), not merely anticipatory for spec 006 M2's chunked deliveries.
**Why:** The counters this milestone needs (`windowReads`, `drawCalls`) are incremented from plain module-level functions in `Spectrogram.tsx` (`getCanvasWindow`, `drawTilesFrame`) that have no service-instance access without significant prop-drilling through several draw functions' parameter lists, for a DEV-only accounting sidecar with no reactive/UI consequence — the signal-ownership pattern used elsewhere in this codebase (one owning service, bridge hooks translate to React) is overkill here since nothing renders from this state. A single chokepoint (`setEntry`) for the byte/tile-count accounting means spec 006 M2 (progressive/chunked analysis) can call `recordEntry` incrementally per chunk without new wiring. This session's own manual self-review (substituting for `/code-review`, gated to human invocation) found the untokened first draft's trackId-keyed `Map<TrackId, number>` genuinely corruptible: two concurrent `analyse()` calls for one track is a real, reachable path in this codebase, not just a theoretical one — the token was added in response. That self-review is itself the kind of substitution the 2026-07-22 process-reorder entry above (and `kb/environment.md`'s corrected 2026-07-21 entry) now says not to do; this entry's fixes stand on their own merits (verified independently, not just asserted), but the review that found them predates that correction.
**Correction (2026-07-22, #539):** the token was still consumed (its pending-start entry deleted) on the *first* `recordEntry` call that matched it, on the (then-true) assumption of one `recordAnalysisStart` per one `recordEntry`. Spec 006 M2's chunked deliveries broke that: `recordEntry` fires once per chunk plus once for the final result, all with the same token, so the first (intermediate) delivery silently consumed it and every later one — including the terminal, `analysisComplete: true` commit whose timing was the actual point — reported `analysisMs: 0`. Missed by this session's own self-review, caught by `/code-review` on #539; see that entry below for the fix (consumption now gated on `analysisComplete`, not on first match).
**Source:** Issue #538 (spec 006 milestone 1).

## 2026-07-22 — Progressive cache writes need the entry to carry its own completion state and to close superseded tiles on every overwrite, not just on invalidate

**Decision:** `TrackSpectrogramEntry` (`SpectrogramCache.ts`) gained `analysisComplete: boolean`, plus a `subscribeToEntry`/per-track listener registry — a consumer that finds an already-cached-but-incomplete entry (one it didn't itself start analysing) can keep listening instead of adopting a permanent stale snapshot. Separately, `setEntry` now closes any of the previous entry's `ImageBitmap` tiles the new `tiles` array doesn't reuse by reference, rather than leaving that only to `invalidate`/`invalidateAll`.
**Why:** Before progressive/chunked analysis (#539), every write to `entries` was a single atomic commit that only ever happened after full success — no consumer needed "final vs. partial," and no overwrite ever discarded live tiles before a track was fully re-analysed. Chunking broke both assumptions at once: `appendChunk` now writes real, partial state into the cache *during* analysis, so (a) a component remounting mid-analysis silently adopted that partial entry as final, freezing its UI on incomplete tiles indefinitely, and (b) a worker-error fallback replacing those partial tiles wholesale leaked the discarded `ImageBitmap`s, since nothing had ever needed to close tiles outside deletion before. Neither was caught during implementation — both surfaced only via `/code-review`, the same session as the `analysisMs` correction above: an "every write is atomic and complete" invariant an unrelated design change (progressive delivery) silently broke, with no test covering either failure mode until review added one.
**Source:** Issue #539 (spec 006 milestone 2), review-fix commit.
