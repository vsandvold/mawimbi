...
## 2026-07-21 — Undo/redo signal-sync effects must diff by reference, not by value

**Decision:** `useTrackEffectsSync` (spec 004 M5, #493) — the effect that pushes a track's persisted `effects` amounts back into the live per-track signal after undo/redo — diffs each track's `effects` object by *identity* (`Map<TrackId, EffectAmounts>` of last-seen references) rather than by comparing the persisted amount against the live signal's current value.

**Why:** Comparing against the live signal seemed natural (`if (signal.value !== persisted) sync`) but races with any in-progress interaction: `projectPageReducer`'s other actions (`DELETE_TRACK`/`MOVE_TRACK` reindexing, `SET_INSTRUMENT` from background classification) all rebuild every `Track` object on every dispatch, but pass unrelated nested fields — `effects` included — through by reference unchanged (`{...track, index: i}` copies the reference, doesn't clone it). Diffing against the live signal meant *any* unrelated tracks-array change re-pushed the last-committed amount into every track's signal, clobbering a slider drag still in progress on a different track. `SET_TRACK_EFFECT` is the only action that ever constructs a new `effects` object, so identity-diffing only fires on a genuine effect change (forward or reverse), never on unrelated churn. Caught in code review before shipping (red-then-green regression test). The same shape generalizes: any effect syncing reducer-owned state back into a live signal for undo/redo should diff the specific nested value it cares about by reference, not by comparing against a signal that a live interaction might be writing to concurrently.
