...
- **One stream, focusable sources.** Music is always perceived as a single combined stream; psychoacoustics lets a listener *focus* on individual sources and events within it, but never hear them outside the mix. The visualization mirrors this: semi-transparent tracks stacked in one cell represent the combined stream, and per-track emphasis/dimming represents selective attention within it — this mapping is how Mawimbi bridges auditory and visual perception of music. Design consequence: features must not force isolation where perception wouldn't — e.g. per-track edit mode never auto-solos the active track, because an edit can only be judged against the mix (owner statement 2026-07-20; applied in spec 004).
