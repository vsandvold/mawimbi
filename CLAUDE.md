...
| `rhythm/` | RhythmAnalyser (essentia `RhythmExtractor2013` + `OnsetRate`), tempo (confidence threshold + predicates), induceBeatGrid, extrapolateTicks, selectRhythmAnchor, anchorBeatTimes, useRhythmAnchor, BeatPulse, RhythmOverlay + rhythmOverlayRenderer |
...
### `Tone.Transport`/`Tone.getContext()`'s static exports go stale after `Tone.setContext()` — resolve fresh, don't hold onto them
A static Tone.js export binds to whatever context was active at *import* time; calling `Tone.setContext()` later doesn't move it. `Tone.Transport` kept driving the old context's transport after a context switch — timeline scrolled with no sound (#133) — and symmetrically, any node created *before* `setContext()` lands on the original default context, which `Transport.start()` on the new context never resumes (the recorder captured silence, #134). Always call `Tone.getTransport()`/`Tone.getContext()` rather than the static exports; `AudioService` resolves both once at construction (since #135) and every service reads its own cached field instead of re-deriving them later.

### `decodeAudioData` detaches the `ArrayBuffer` it's given
Calling it consumes the buffer — persisting that same reference afterward silently writes empty bytes. Uploads once appeared to save to IndexedDB but restored as nothing (#260). Clone the buffer (`.slice(0)`) before decoding whenever the raw bytes are also stored elsewhere.

### A disconnected Web Audio subgraph stops rendering entirely — its params and any buffered audio freeze mid-value
Bypass-via-disconnect (the pattern spec 004's per-track effect chains use) means a node reconnecting from bypass still holds whatever wet/feedback values it had when it was disconnected, plus up to a delay line's worth of stale buffered audio. `EffectsChain` therefore *snaps* (never ramps) params on (re)activation — the node was silent going in, so there's no zipper-noise risk — which also instantly attenuates any stale delay-line replay down to the new amount (#492). Ramp only for changes made while the node is already live and connected.

### `Tone.UserMedia.open()` hardcodes its `getUserMedia` constraints, with no override API
Low-latency mic capture needs echo cancellation/noise suppression/AGC off, mono, and `latency: 0` — none of which `Tone.UserMedia.open()` exposes a way to set. `MicrophoneService` instead calls `navigator.mediaDevices.getUserMedia()` directly with those constraints and injects the resulting stream into `UserMedia`'s private `_stream`/`_mediaStream` fields (#219). Fragile across Tone upgrades by construction — recheck this workaround on every Tone version bump.

### `Tone.BitCrusher`'s constructor options type omits `wet`, though the node has one
Its two overloads are `(bits?: number)` and `(options?: Partial<BitCrusherWorkletOptions>)` — and `BitCrusherWorkletOptions` descends from `ToneAudioWorkletOptions`, *not* from `EffectOptions`, so `wet` is missing from the type even though `BitCrusher extends Effect` and the runtime constructor accepts it (`Effect`'s constructor reads `options.wet`). Passing `{ bits, wet, context }` is a `tsc` error, not a runtime one. Set it on the param instead — `crusher.wet.value = wet` — rather than casting the options object; both `EffectsChain.ensureNode` and `renderTrackOffline`'s `buildEffectsChain` do this (#558). Worth checking the declared option type before assuming any worklet-backed Tone effect takes the full `Effect` option set.
...
### A React state update inside `useScrubberScroll` kills the Scrubber's signal subscriptions
Adding *any* `useState` whose setter fires from an effect in `useScrubberScroll` (directly, or via a hook it calls) leaves `Scrubber` no longer re-rendering on `playbackState` changes — the audio engine keeps responding while the component reads stale values, which is the #114 class exactly. Measured while building spec 008 M5 (#571): the natural implementation of the arrival pulse called `useRhythmAnchor` from this hook, and its mount effect's single `setCached` — even the no-op `setCached(null)` when there is no anchor — broke `Scrubber.test.tsx`'s scroll-sync and scrub-to-pause tests. Both surfaced as unrelated-looking assertion failures (`scrollTop` still 0; `isPlaying` still true), never as a reactivity error, and bisecting to a bare `useState` + `setState`-in-effect is what identified it; a *bailed-out* update (setting the same value) breaks it just as a real one does. Every piece of state this hook needs is therefore held in refs. When a per-frame consumer needs derived React-owned data, derive it without React — `features/rhythm/anchorBeatTimes.ts` is the worked example: the same derivation `useRhythmAnchor` does for the rendering path, as a memoizing plain class the rAF loop calls. Don't "clean it up" into a hook.

### An idle/resting redraw is not necessarily a playback discontinuity
`renderLoudnessMeterIdle` doubles as the reset signal for the playhead meter's per-frame ballistics (`BarSmoother`, then `BeatPulse`), but two of its call sites — `Playhead.tsx`'s ResizeObserver and its `meterWidthFraction` layout effect — are *layout* changes that also fire during playback. That conflation was free while bar ballistics were the only client (restarting them for one frame is invisible) and became a bug the moment `BeatPulse` joined: resetting it drops the envelope's *phase*, so the next frame has no interval to have crossed a beat in — drag-resizing the window mid-playback suppressed the flare for the whole drag, and opening a bottom sheet dropped a beat (#571, `/code-review` on PR #588). Hence the split: `renderLoudnessMeterIdle` (discontinuity — resets) versus `repaintLoudnessMeterIdle` (layout — same pixels, ballistics untouched). Any new per-frame state added to this canvas has to pick a side, and `Playhead.test.tsx` pins which entry point each call site reaches.
...
### Never put a `<button>` in a `<label>` that also holds a Radix control — the label forwards every click to it
A `<label>` resolves its *control* to its first **labelable** descendant (`button`, `input`, `select`, `textarea`, …), and a click anywhere in the label that isn't interactive content fires a synthetic click on that control. Radix's slider root is a `<span role="slider">` — **not** interactive content and not labelable — so a label wrapping one is inert *until* a labelable element joins it, at which point every click and drag on the slider (and on the label's own text) silently activates that element. Shipped exactly once: the effects drawer's row was a `<label>`, and adding the Echo subdivision buttons made `label.control` the quarter-note button, so every Echo fader drag toggled tempo sync — changing the audible delay, adding an undo entry and kicking off an offline re-render (#560, `/code-review` on PR #582). The rows are plain `<div>`s now, which costs nothing: the label never associated with the slider anyway. **jsdom does not model this forwarding** (`element.click()` on the slider span does nothing), so it cannot be caught by unit tests behaviourally — assert the structure at unit level (`group.closest('label')` is null) and the behaviour in e2e. Reproduced in Chromium against a minimal `label > span[role=slider] + button` page before fixing.

### A Radix Slider caches its bounding rect at slide start — any layout change mid-drag corrupts the mapping
`SliderHorizontal` stores the slider's `ClientRect` in a ref on slide start and clears it only on slide end, so every pointer event for the rest of that gesture maps to a value through the *old* geometry. Anything that resizes the slider mid-drag — a sibling mounting next to it, a container reflow — silently skews the committed value with no error: the effects drawer's subdivision control appearing when a tempo estimate lands (tens of seconds after upload, i.e. while a user is plausibly already dragging) narrowed the Echo slider, and a release at the visible right end committed ~66 instead of 100 (#560, `/code-review` on PR #582). Hold layout that depends on async state until the gesture ends — `EffectsBottomSheet.tsx` latches the control's visibility off the same wrapper pointer lifecycle that drives `endDrag`.

### A CSS Grid `auto` track does not collapse its gap when it holds nothing
An empty `auto` track has zero width but the `gap` *before* it still applies, so declaring an extra column on a rule shared by several rows shortens the other rows' content by the gap and misaligns their right edges. Measured in Chromium while adding the Echo subdivision column to `.effects-bottom-sheet__effect`: `56px 1fr auto` gave the non-Echo sliders 280px ending at x=356, versus 292px ending at x=368 under `56px 1fr` (#560, `/code-review` on PR #582). Put the extra track on a modifier class scoped to the row that actually has the item (`.effects-bottom-sheet__effect--synced`).

### `Tone.FeedbackDelay`'s `maxDelay` defaults to 1 second, and the native `DelayNode` clamps rather than errors
`maxDelay` sizes the underlying delay line at construction and cannot be raised afterwards; a `delayTime` beyond it is silently clamped, so the echo plays at the wrong time with nothing reporting a problem. This became reachable with tempo-synced Echo (#560): a quarter note below 60 BPM exceeds 1 s, and essentia estimates down to 40 BPM (1.5 s). Every `FeedbackDelay` in this app is constructed with an explicit `maxDelay: ECHO_MAX_DELAY_SECONDS` (`echoSync.ts`, 2 s — a quarter note at 30 BPM); `selectEchoDelaySeconds` clamps to the same constant, so the two can't drift apart.
...
### A per-track canvas's `trackBase` already carries `startTime` — a renderer that applies it again needs the offset removed first, and the two halves must cancel
`Spectrogram.tsx`'s container sets `marginBottom: startTime × pps`, so `getTrackBase` returns the position of that track's *buffer* time 0, not project time 0 — which is why `drawPianoRoll` takes raw (track-buffer-relative) melody times and adds nothing. The rhythm layer's shared renderer does the opposite: `drawOnsetTicks`/`drawBeatRungs` apply `startTime` themselves, because `RhythmOverlay`'s own container is a zero-height grid item sitting at project time 0 (#569). Drawing ticks into a *track's* canvas therefore has to convert back — `writeRhythmTicks` derives `timeZeroY = trackWin.height − trackBase + startTime × pps` and then passes `startTime` on, so the two applications cancel (#570). Deleting either half alone is correct for every uploaded track (`startTime: 0`) and wrong by the whole start time for an overdub — the #484 class again, and a shape that reads like a redundant round trip until you check which zero each side means. Both halves are pinned by `Spectrogram.test.tsx`'s overdub test, which asserts a track at `startTime: 3` marks the same canvas rows as one at 0; jsdom's zero `offsetTop` collapses the margin exactly as a zero start time does in the browser, so that equality is the invariant.

### A `TimelineRenderLoop` registration's `peekDirty` must short-circuit on exactly the states its `write` declines to handle
`peekDirty` compares the current inputs against sentinels that only `write` updates, so any state `write` returns early from is a state whose sentinels never advance — and the callback then reports dirty on *every* frame, forever. That doesn't just cost this callback a redraw: `runFrame` treats one dirty callback as a dirty frame, so the whole loop stops idling and every mounted track pays the shared window read (mawimbi#541's entire point). `Spectrogram.tsx` documents one instance (its `noteCount` sentinel starts at `0`, not `-1`, because `writeMelodyOverlay` only runs when there are notes); `RhythmOverlay.tsx` hit the other direction in #569 — its `write` skips everything when there's no confident anchor so an anchorless project costs nothing, which left `peekDirty`'s `NaN`/`null` sentinels permanently mismatched until it gained the same guard. The rule when adding or changing a registration: for every early return in `write`, ask what `peekDirty` reports in that state, and mirror the condition. `e2e/spectrogram-render-loop.spec.ts`'s idle-frame test is the guard (it caught this on CI, not locally) — re-run it after touching any registration's write-side early returns, not just after adding a new registration. The cheapest way to make the two agree is to have `write` advance its sentinels *before* any early return, so there is no state it declines to handle without recording that it did.

The **false-negative** direction is a separate trap with a separate fix: a dirty signal the callback cannot derive from its own inputs. A container resize (drawer open/close, rotation) changes none of the engine time, parameters, or data a `peekDirty` compares, so a canvas sized to its own box goes stale with no frame scheduled to fix it — visible only while stopped, since playback marks every frame dirty anyway (`StringMode.tsx`, `/code-review` on mawimbi#594). Measuring the box inside `peekDirty` is not the fix: it runs on every idle frame and must not force layout. Use a `ResizeObserver` that sets a ref flag, cleared by `write` once the new box has actually been drawn at.

### `SharedCanvasWindow` is sized for the *tilted* runway, not for the viewport
`TimelineRenderLoop`'s shared `win.height` is the runway's **pre-transform local-Y span** — the range that can project into view under `rotateX`, which is several times the viewport height because the tilt pulls content from far above the box back down into it. Measured at 2719 px against a 792 px visible box on a 420×860 viewport. Every existing registration is a per-track canvas *inside* the tilt, where that is exactly right. A callback that renders **outside** the tilt (spec 009's String mode is the first) and sizes its backing store from `win.height` gets it CSS-scaled down into its own box, squashing everything by that ratio with no error anywhere. Such a registration must read its own `clientWidth`/`clientHeight` in the `measure` phase — the one DOM read the shared window cannot hoist, same category as `getContentOffsetTop`.

### A canvas colour string built from `NaN` *throws*, and in the shared loop it takes down every track's frame
`addColorStop`/`fillStyle` reject `rgb(NaN, NaN, NaN)` by throwing a `SyntaxError`, not by ignoring it — and because `TimelineRenderLoop` runs every registered `write` in one try-less loop, one bad sample from one track aborts the whole frame for every mounted canvas. Non-finite inputs are not exotic here: a silent frame legitimately has no pitch (no melody note, no contour, and a spectral centroid of zero), so any derived-value → colour path needs its clamps to be `Number.isFinite`-guarded at the *source* rather than relying on `Math.min`/`Math.max`, which propagate `NaN` silently (`normalizePitch`, `clamp01` and `encodeChannel` in `features/stringmode/`, mawimbi#593).
...
### A restore path must read each persisted row independently — don't nest a small row's read inside a large one's presence
`useSpectrogramCache`'s `loadOrAnalyse` loads a track's spectrogram, melody, and rhythm rows in one `Promise.all`, but originally only *used* `storedRhythm` inside the `if (storedSpectrogram)` branch — so whenever the spectrogram row was missing, a perfectly good rhythm row was discarded and the multi-second essentia pass re-ran on every load, forever, with nothing anywhere reporting a problem. The rows aren't written together: each is a separate fire-and-forget promise, and the megabyte-scale spectrogram write is the one that realistically fails (quota) while the kilobyte rhythm row persists fine. Fixed in mawimbi#568 by hoisting the decision into a `restoreOrExtractRhythm()` called from both branches (it must run *after* the track's cache entry exists — `setRhythm` writes onto it). `storedMelody` had the identical nested shape — re-running Basic Pitch (~10s of TF.js on a 2-second fixture, on the *shared* spectrogram worker, so it also delayed every analysis queued behind it) on every load — and was fixed the same way in mawimbi#579 with a `restoreOrExtractMelody()`; both now share a `refreshEntry()` closure, since `setMelody`/`setRhythm` mutate the cached entry in place and React needs a fresh object reference to re-render. Any new per-track store added to `TRACK_DATA_STORES` needs its own independent read for the same reason — the restore path is the second half of adding a store, not a follow-up.

### `restoreTrack` fires the same `onTrackCreated` hook `createTrack` does — anything hooked there re-runs on every page load
`TrackService.createTrack` and `restoreTrack` both end with `this.onTrackCreated?.(trackId, audioBuffer)`, and `AudioService` hooks instrument classification to it. A restored track therefore re-ran the full essentia + ONNX pass on every reload to recompute a label already sitting on its track record, because the service's cache is in-memory only (there is no `classifications` store). The fix shape is a persisted-result short-circuit *seeded before* the restore, not a check inside the hook: `useRestoreAudio` calls `classificationService.hydrate(trackId, track.instrument)` before `restoreTrack`, and `classify()`'s existing `done` check does the rest (mawimbi#584). Any future per-track background job hooked to `onTrackCreated` needs the same treatment — the hook alone cannot tell a first upload from a reload, and nothing about the call site hints that it runs twice in a track's life.

### Cache-backed background results have no signal — subscribe; and notify with a *copy*, or React silently ignores you
Two halves of the same trap, both from #559. **First:** `useClassificationSync`'s render-time poll works only because `InstrumentClassificationService` is signals-backed, so a completed round-trip re-renders the workstation by itself. `SpectrogramCache` owns no signal, so the same shape applied to melody/rhythm/tempo would sit there reading stale data until some unrelated render happened to run it — `useTempoSync` uses `subscribeToEntry` instead, with one synchronous read before subscribing to cover data that landed first. **Second:** `setMelody`/`setRhythm` mutate the cache entry *in place*, so notifying subscribers with that same object reference is indistinguishable from not notifying at all — a React subscriber that pushes it into state (`useSpectrogramCache`'s mid-analysis branch, whose effect returned early and has no `refreshEntry` of its own) is bailed out by `Object.is`, and the melody never reaches the screen despite being cached and persisted. `SpectrogramCache.notify` therefore passes `{ ...entry }`; `tiles` keeps its identity, so `Spectrogram.tsx`'s reference dirty check still works. Any future in-place write onto a cached entry needs the same copy-on-notify.

### An idempotence guard written with `===` never terminates on `NaN`
A "have I already synced this?" check that compares incoming numbers against stored ones (`useTempoSync`'s `track.tempo?.bpm === rhythm.bpm`) is always false for `NaN`, so a single non-finite value turns dispatch → re-render → dispatch into an unterminating loop — a frozen tab, not a wrong number. The guard is to reject non-finite input before the comparison, not to fix the comparison. Worth doing even when the producer looks safe: real essentia was probed on silence, DC, near-silence, a single click and a sub-second tone and never returned `NaN` (it returns finite values or throws `Empty vector input`), but the cost of the check is one line and the cost of being wrong is the whole tab (#559, `/code-review`).

### A background job's completion callback must check the entity still exists before writing back
A `.then()`/`.catch()` callback on a long-running background operation (worker round-trip, async extraction) that fires after the user has already deleted the track it was working on will still run — cancellation flags scoped to a React effect (`cancelled` in `useSpectrogramCache.ts`) don't reach a callback chain started by a *different* effect instance, and don't stop the promise itself from resolving. `extractAndCacheMelody`'s worker-completion handler checks `spectrogramCache.getEntry(trackId)` before calling `saveMelodyData`/`setMelody` — an empty/missing entry means `useDeleteTrackAudio`'s `invalidate(trackId)` already ran, so the track is gone and the write is skipped (mawimbi#540 follow-up; without this check, a track deleted mid-extraction got a fresh orphaned `melodies` row written back after cleanup already ran — the same class of bug the milestone's IndexedDB audit was fixing, just via a race instead of a missing delete call). `TranscriptionService.ts`'s analogous background-save path has the same shape and is **not yet guarded** — tracked as #580, not fixed as of #540.

**The entry-absence guard has a precondition that is easy to violate: the entry must already exist when the job is *started*.** The spectrogram worker is a single FIFO — one request at a time, each blocking the queue for its whole CPU pass — and `useSpectrogramCache` awaits three IndexedDB reads before posting anything. A new consumer that posts *synchronously* from its own effect therefore jumps ahead of the spectrogram request for the same track, and its result comes back before any cache entry exists. "No entry" then means "not analysed yet", not "deleted", so the guard discards a perfectly good result and (if the consumer also has an in-flight claim) never retries. Both halves are fixed by the same move: wait for the track's `analysisComplete` via `SpectrogramCache.subscribeToEntry` before posting, which restores the guard's meaning *and* stops the new work head-of-line-blocking the spectrogram, melody and rhythm passes it depends on (`useStringEnvelopes.ts`, `/code-review` on mawimbi#594).
