...
| the `mawimbi-ribbons.jsx` prototype | Not in the repository and not supplied with the prompt. Its parameter values are carried into the parameter table below **as reported in the prompt**, unverified. |
...
>
> At 4 tracks, halve it. The budget is stated so a milestone can falsify it; if measurement lands above 8 ms, the escape hatches in order are: drop `N` to 96, drop gradient stops to 16, then WebGL.
>
...
> **Decision.** A new `extractEnvelopes` step runs in the **existing spectrogram worker** (`spectrogram.worker.ts`, alongside `AnalyseRequest` / `MelodyRequest` / `RhythmRequest`) over the CQT frames `analyseCQTChunked` has already computed, and emits five Float32Array envelopes at the CQT's own 25 ms hop:
>
...
| Pan | **Undecided at Now scale** | No `Track.pan` field exists and #168 is open, so there is no value to encode. Currently exiled to a gutter in the prototype. Recorded as unresolved. |
...
6. **Pan.** Nothing to encode until #168 lands. When it does: vertical lane offset, ribbon skew, and a per-ribbon gutter are the candidates; the gutter is what the prototype does and is the least integrated.
