...
Re-anchor the loudness-meter playhead so its **bottom edge sits on the playhead line** (today the box is centered on it), reduce its height accordingly, and evolve its rendering from raw CQT bars into a musically aware display: per-semitone (12-TET) frequency bars, loudness weighting that emphasizes fundamental-range content over spectral extremes, a sparkle/welding burst where a transcribed note crosses the line, and an onset-driven pulse of the meter frame. Owner request (2026-07-19), building on the settled runway geometry (#443–#464): the geometry itself — playhead position, resize, drawer open/close re-solving — is explicitly out of scope because it already works.

## Grounding

- KB `kb/product.md`: the spectrogram/runway is the product's visual identity; the vertical timeline exists so music "comes towards you" into the playhead — effects that fire *at the line* reinforce the core metaphor. Mobile first; visual polish of the runway area is actively invested in (#443–#469).
...
4. **Fundamental emphasis.** Bar intensity is weighted by a fixed perceptual curve peaking over the melody range (~C2–C6): equal CQT byte energy renders visibly taller mid-range than at the spectral extremes.
...
### Q3 — Fundamental emphasis

> **Decision:** A fixed perceptual weighting curve (pure function, named constants) applied to semitone bar intensities, peaking across ~C2–C6 and rolling off toward both extremes; the unused `loudness` parameter in the render path is removed rather than repurposed.
> **Rationale:** Matches the request literally ("more than the high/low frequency content"), is deterministic and unit-testable, and costs O(bins) per frame.
> **Dissent:** A harmonic-support boost (poor-man's harmonic product spectrum: boost a bar whose 2f/3f bars also carry energy) would emphasize *true* fundamentals rather than a fixed band, but risks per-frame flicker on byte-quantized data. Recorded as a follow-up candidate if the fixed curve reads as dumb on real material.
...
| 4 | Equal byte energy at C4 vs C1/C8 renders strictly taller at C4; curve is monotone toward its peak; weights in [0,1] | unit | `__tests__/fundamentalWeighting.test.ts` |
...
4. **Fundamental weighting** — weighting curve + tests; drop the dead `loudness` render parameter.
...
- **Does the fixed weighting curve read as "fundamental emphasis" on real multi-track material, or does it need harmonic support (Q3 dissent)?** Resolved by the Milestone 6 QA checklist; the follow-up path is recorded in the Design section.
